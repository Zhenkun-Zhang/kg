...
directApis = ['tensors', 'tensor_attributes', 'amp', 'autograd', 'library',
              ...
              'hub', 'jit', 'motinor', 'special', 'torch.overrides', 'package',
...
def solve(version='stable', max_workers=8):
    ...
    def handle_api(api):
        ...
        for link in links:
            ...
            if methodFlag:
                apinum = getDL(r.text, path, version, apinum)
    ...
    def handle_direct(api):
        ...
        if not r:
            failed.append(url)
            return
        apinum = getDl(r.text, os.path.join(save_path, api), version, apinum)
    ...
    print(f"[SUMMARY] Total: {apinum}, Failed: {len(failed)}")
    if failed:
...
def getDl(req, path, version, apinum):
    soup = BeautifulSoup(req, 'html.parser')
    for cls in ['function', 'attribute', 'class']:
        for dl in soup.find_all('dl', class_=cls):
            ...
            if cls == 'class' and methodFlag:
                apinum = getDL(str(dl), path, version, apinum)
    return apinum
