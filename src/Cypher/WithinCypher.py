...
def deal_class_and_op(frame, version, name, type, desc, args_list):
    ...
    for i in range(len(classes) - 1):
        class_i = classes[i]
        print('merge (: module {framework: "%s", name: "%s", version: "%s"});' % (frame, class_i, version))
        if i == 0:
            print('match')
            print(' (m1: framework {name: "%s", version: "%s"}),' % (frame, version))
            print(' (m2: module {framework: "%s", name: "%s", version: "%s"})' % (frame, class_i, version))
            print('merge (m1) -[: classOfFramework {name: "%s"}]-> (m2);' % class_i)
        else:
            print('match')
            print(' (m1: module {framework: "%s", name: "%s", version: "%s"}),' % (frame, classes[i - 1], version))
            print(' (m2: module {framework: "%s", name: "%s", version: "%s"})' % (frame, class_i, version))
            print('merge (m1) -[: subClassOfClass {name: "%s"}]-> (m2);' % class_i)
...
if __name__ == '__main__':
    main()
# python src/Cypher/WithinCypher.py 