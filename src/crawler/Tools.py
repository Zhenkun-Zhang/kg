...
def dealDefault(default):
    default = default.replace("ms_type", "mindspore")
    i = default.strip()
    if len(i) == 0:
        i = ""
    else:
...
def getInformation(link, api_num, version, path):
    print_hit(link)
    try:
        # Add verify=False to bypass SSL verification and suppress warnings
        import urllib3
    ...
    for dl_operator in dls:
        ...
        print(api_num, apiName, api_type)
        ...
        jsonDumps(apiName, api_type, desc, version, path, Parameters, Returns, Supported_Platforms, Raises, args_list)
    return api_num
...
def jsonDumps(apiName, api_type, desc, version, path, Parameters, Returns, Supported_Platforms, Raises, args_list):
    jsDict = {}
    jsDict['api'] = apiName
    jsDict['type'] = api_type
