...
def safe_request(url):
    try:
        ...
        print(f"[SSL Warning] Certificate verification failed for {url}")
        print(f"Reason: {e}")
        print("[INFO] Retrying without certificate verification...")
        try:
...
def jsonDumps(api, apiType, params, returns, desc, version, path, arg_list):
    ...
    # 从 params 中提取所有参数名称，确保 args_list 不为空
    if len(arg_list) == 0:
        args_list = [param['name'] for param in myParams]
    else:
        args_list = arg_list
